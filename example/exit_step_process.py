...
parser.add_argument('--epochs', type=int, default=10, help='number of epochs, (default: 1000)')
...
gipps_acc_values = [1, 1.5, 2, 3]
gipps_decel_values = [-3, -2, -1]
gipps_tau_values = [0.1, 1]
gipps_s0_values = [1, 2]
...
hyperparameter_combinations = product(gipps_acc_values, gipps_decel_values, gipps_tau_values, gipps_s0_values)

for combination in hyperparameter_combinations:
    gipps_params=combination
    print('gipps_params',gipps_params)
    for m in range(len(hws)):
    # state = np.clip((state_ - state_rms.mean) / (state_rms.var ** 0.5 + 1e-8), -5, 5)
        ...
        for n_epi in range(args.epochs):
            ...
            veh_name3 = 'vehcleg3_'
            veh_name4 = 'vehcleg4_'
            ## problem is it will stuck in the middle, need to investigate when will it swtich lane
            distance=60
            distance2=70
            departspeed=10
            vels=[]
            ...
            for t in range(args.horizon):
                ...
                # if t==102:
                #     lane=2
                next_state_, reward_info, done, collision = env.step([lane,0],sumo_lc=True,sumo_carfollow=True,stop_and_go=False,car_follow='Gipps',lane_change='SECRM',gipps_params=gipps_params)
        ...
        print('collision rate',collide/args.epochs)
        result={
            'gipps_param':gipps_param,
            'hw':hws[m],
            'vel':np.mean(vel_lst),
            'jerk':np.mean(jerk_lst),
            'stuck': stuck/args.epochs,
            'collide':collide/args.epochs

        }
        datas.append(result)
np.save('results/gipps_main.npy',datas)